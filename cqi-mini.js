...
function miniCqiKpi1PresentationChart(monthly,years,inScope){
 ...
 const w=Math.max(900,months.length*135+190),h=430,left=85,right=45,top=151,base=342,plotH=base-top,maxCount=Math.max(2,...months.map(ym=>monthly.get(ym).total)),spacing=(w-left-right)/months.length;
 const total=months.reduce((n,ym)=>n+monthly.get(ym).total,0),passed=months.reduce((n,ym)=>n+monthly.get(ym).passed,0),pct=100*passed/total;
 const guides=[0,maxCount].map(value=>{const y=base-plotH*value/maxCount;return `<line x1="${left}" x2="${w-right}" y1="${y}" y2="${y}" stroke="#e5edf3"/><text x="${left-14}" y="${y+5}" text-anchor="end" font-size="13" fill="#617789">${value}</text>`}).join('');
 const bars=months.map((ym,i)=>{const item=monthly.get(ym),rate=100*item.passed/item.total,x=left+spacing*(i+.5),barW=Math.min(76,spacing*.47),riskH=plotH*item.passed/maxCount,otherH=plotH*(item.total-item.passed)/maxCount,label=`${String(Number(ym.slice(5))).padStart(2,'0')}/${Number(ym.slice(0,4))+543}`;return `<rect x="${x-barW/2}" y="${base-riskH}" width="${barW}" height="${riskH}" rx="5" fill="#278f79"/>${otherH?`<rect x="${x-barW/2}" y="${base-riskH-otherH}" width="${barW}" height="${otherH}" rx="5" fill="#d98b46"/>`:''}<text x="${x}" y="${base-riskH-otherH-12}" text-anchor="middle" font-size="18" font-weight="700" fill="#27475f">${item.passed}/${item.total} แผน</text><text x="${x}" y="${base+28}" text-anchor="middle" font-size="15" font-weight="700" fill="#29475f">${label}</text><text x="${x}" y="${base+50}" text-anchor="middle" font-size="13" fill="${rate===100?'#278f79':'#b16b30'}">${rate.toFixed(1)}%</text>`}).join('');
 return `<div class="mini-cqi-chart-toolbar no-print"><button type="button" class="btn btn-light" onclick="exportMiniCqiChart('kpi1')">ดาวน์โหลดกราฟรายเดือน PNG</button></div><div class="kpi-year-chart-scroll mini-cqi-kpi1-chart"><svg class="kpi-exec-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="KPI 1 จำนวนแผนออกหน่วยที่พบความเสี่ยงรายเดือนพร้อมสัดส่วนและจำนวนแผน"><rect x="5" y="5" width="${w-10}" height="${h-10}" rx="18" fill="#fff" stroke="#e7eff5"/><text x="${left}" y="42" font-size="21" font-weight="700" fill="#234969">KPI 1 · แผนออกหน่วยที่พบความเสี่ยง</text><text x="${left}" y="70" font-size="14" fill="#526b7d">จำนวนแผนรายเดือน · ตัวเลขเหนือแท่ง = แผนพบความเสี่ยง / แผนที่บันทึก</text><text x="${left}" y="111" font-size="19" font-weight="700" fill="#278f79">พบความเสี่ยง ${passed}/${total} แผน (${pct.toFixed(1)}%)</text><rect x="${w-232}" y="93" width="13" height="13" fill="#278f79"/><text x="${w-212}" y="105" font-size="12" fill="#526b7d">พบความเสี่ยง</text><rect x="${w-117}" y="93" width="13" height="13" fill="#d98b46"/><text x="${w-97}" y="105" font-size="12" fill="#526b7d">ไม่พบ</text><text x="${left-12}" y="${top-13}" text-anchor="end" font-size="12" fill="#617789">แผน</text>${guides}${bars}<text x="${left}" y="${h-16}" font-size="12" fill="#657a8b">เฉพาะเดือนที่มีแผนบันทึก · เป้าหมายพบความเสี่ยง 100% · เดือนอื่นไม่นับเป็น 0%</text></svg></div>`;
}
