...
function miniCqiClassifyFamilies(rows){
  ...
  return [...families].map(([key,members])=>{
    ...
    return {key,bagNumber:representative.bagNumber,productType:representative.productType,site:representative.donateSource,
      ...
      reason:missingExpiry?'ถุงที่ใช้ยังไม่มีวันหมดอายุในฐานข้อมูล':''};
  });
}
...
function miniCqiSetPeriod(){
  ...
  miniCqiPeriod={from,to};miniCqiProductScope=document.getElementById('miniCqiProductScope')?.value==='all'?'all':'red';loadMiniCqi();
}
async function loadMiniCqi(){
  ...
  try{
    ...
    const details=miniCqiClassifyFamilies(families.rows||[]).filter(row=>row.cohortDate>=range.from&&row.cohortDate<=range.to&&(miniCqiProductScope==='all'||row.key.endsWith('||RBC')));
    ...
    const missingExpiryUsed=details.filter(row=>row.missingExpiry).length;
    miniCqiResult={range,outings,months:dep.months,previousMonths:previous.months,details,counts,k1,k2,k3,productScope:miniCqiProductScope,missingExpiryUsed};
    ...
    box.innerHTML=`<div class="simple-page-head"><div><h1>ผล CQI: Mini Mobile Blood Donation</h1><div class="small-muted">แสดงข้อมูลตามช่วงเดือนที่รับเข้าหรือวันที่ออกหน่วย</div></div><button class="btn btn-light no-print" onclick="exportMiniCqi()">ส่งออก CSV รวมพร้อมสูตร</button></div>
      <div class="simple-panel mini-cqi-filters no-print"><label>จากเดือน <input type="month" id="miniCqiFrom" value="${miniCqiHtml(miniCqiPeriod.from||miniCqiMonth(range.from))}"></label><label>ถึงเดือน <input type="month" id="miniCqiTo" value="${miniCqiHtml(miniCqiPeriod.to||miniCqiMonth(range.to))}"></label><label>KPI 3 · ผลิตภัณฑ์<select id="miniCqiProductScope" class="form-control"><option value="red" ${miniCqiProductScope==='red'?'selected':''}>เลือดแดง (หลัก)</option><option value="all" ${miniCqiProductScope==='all'?'selected':''}>ทุกผลิตภัณฑ์ (สำรวจ)</option></select></label><button class="btn btn-main" onclick="miniCqiSetPeriod()">แสดงผล</button></div>
      ...
      <div data-mini-panel="overview" class="simple-kpi-grid blood-kpi-main-grid mb-3">${miniCqiCard('1 · ตรวจความพร้อมก่อนเริ่ม',k1.passed,k1.total,'100%',k1.passed===k1.total,`${k1.missing.length} ครั้งยังไม่ยืนยัน · ${k1.late.length} ครั้งบันทึกย้อนหลัง`)}${miniCqiCard('2 · กาชาด Routine · เลือดแดง',k2.num,k2.den,'≤ 20%',k2.num/k2.den<=.20,'ตัดเลือดหายาก/จำเป็นตามนิยามหน้า KPI เดิม')}${miniCqiCard('3 · ใช้เลือดแดงก่อนหมดอายุ',k3.num,k3.den,'≥ 95%',k3.num/k3.den>=.95,`${counts['รอติดตามผล']} รอติดตาม · ${counts['ต้องตรวจสอบ']} ต้องตรวจสอบ · ${missingExpiryUsed} ถุงใช้แต่ขาดวันหมดอายุ`,missingExpiryUsed>0)}</div>
...
function exportMiniCqiSection(key){
 ...
 const rows=[['CQI '+names[key],r.range.from,r.range.to],['ตัวตั้ง','ตัวหาร','ร้อยละ','เป้าหมาย','สูตร'],[num,den,key==='kpi3'&&r.missingExpiryUsed?'รอตรวจสอบ':miniCqiPct(num,den),{kpi1:'100%',kpi2:'≤20%',kpi3:'≥95%'}[key],formulas[key]]];
 if(key==='kpi2')rows.push(['ผลิตภัณฑ์','เลือดแดง Routine (ฐานเดียวกับหน้า KPI เดิม)']);
 if(key==='kpi3')rows.push(['ผลิตภัณฑ์',r.productScope==='red'?'เลือดแดง (ผล CQI หลัก)':'ทุกผลิตภัณฑ์ (สำรวจเพิ่มเติม)'],['ถุงที่ใช้แต่ไม่มีวันหมดอายุ',r.missingExpiryUsed]);
...
function exportMiniCqi(){
 const r=miniCqiResult;if(!r)return;
 const lines=[['ผล CQI Mini Mobile Blood Donation',r.range.from,r.range.to],['KPI','ตัวตั้ง','ตัวหาร','ร้อยละ','เป้าหมาย','สูตร'],['KPI 1',r.k1.passed,r.k1.total,miniCqiPct(r.k1.passed,r.k1.total),'100%','ตรวจครบและยืนยันพร้อมก่อนเริ่ม / ออกหน่วยที่ลงทะเบียนทั้งหมด'],['KPI 2 · เลือดแดง',r.k2.num,r.k2.den,miniCqiPct(r.k2.num,r.k2.den),'≤20%','กาชาด Routine / รับเข้า Routine ทั้งหมด; ไม่รวม Rare'],['KPI 3 · '+(r.productScope==='red'?'เลือดแดง (หลัก)':'ทุกผลิตภัณฑ์ (สำรวจ)'),r.k3.num,r.k3.den,r.missingExpiryUsed?'รอตรวจสอบ':miniCqiPct(r.k3.num,r.k3.den),'≥95%','ใช้/จ่าย/ส่งต่อ / (ใช้/จ่าย/ส่งต่อ + หมดอายุ); นับถุงต้นทาง'],['KPI 3 ถุงที่ใช้แต่ไม่มีวันหมดอายุ',r.missingExpiryUsed],[],['เดือน','กาชาด Routine','Routine รับเข้าทั้งหมด','อัตรา']];
