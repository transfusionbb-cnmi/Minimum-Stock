...
function miniCqiSummary(plans,actualOutings){
 const today=getTodayYmd(),byKey=new Map();
 ...
 const events=actualOutings.filter(row=>row.outing_date<=today).map(row=>{const matching=byKey.get(miniCqiEventKey(row.outing_date,row.site))||[];return {...row,plans:matching,passed:matching.some(miniCqiPlanPassed)}});
 return {passed:events.filter(row=>row.passed).length,total:events.length,missing:events.filter(row=>!row.plans.length),risk:events.filter(row=>row.passed),events};
}
function miniCqiKpi1MonthlyTable(monthly,years,inScope){
 const rows=years.flatMap(year=>Array.from({length:12},(_,i)=>`${year}-${String(i+1).padStart(2,'0')}`)).filter(month=>inScope(`${month}-01`));
 return `<details class="mini-cqi-help mini-cqi-coverage" open><summary>ผลรายเดือน (${rows.length} เดือน)</summary><div class="table-responsive"><table class="table simple-table"><thead><tr><th>เดือนออกหน่วย</th><th>พบหมู่เสี่ยง</th><th>ออกหน่วยจริงที่พบในข้อมูล</th><th>อัตรา</th></tr></thead><tbody>${rows.map(month=>{const row=monthly.get(month),year=Number(month.slice(0,4))+543;return `<tr><td>${month.slice(5)}/${year}</td><td>${row?row.passed:'—'}</td><td>${row?row.total:'—'}</td><td><b>${row?miniCqiPct(row.passed,row.total):'ไม่มีข้อมูลออกหน่วย'}</b></td></tr>`}).join('')}</tbody></table></div></details>`;
}
function miniCqiKpi1PresentationChart(monthly,years,inScope){
 ...
 const w=Math.max(820,months.length*75+96),h=300,left=64,base=244,top=98,step=(w-left-32)/Math.max(1,months.length);
 const columns=months.map((month,i)=>{const record=monthly.get(month),x=left+i*step+step/2,percent=record?record.passed/record.total*100:0,height=(base-top)*percent/100,color=record?(record.passed===record.total?'#278f79':'#dd8b3c'):'#dce7ee';return `<rect x="${x-17}" y="${record?base-height:base-5}" width="34" height="${record?Math.max(height,2):5}" rx="4" fill="${color}"/><text x="${x}" y="${record?Math.max(91,base-height-13):base-17}" text-anchor="middle" font-size="12" font-weight="700" fill="${record?'#264963':'#60798c'}">${record?miniCqiPct(record.passed,record.total):'—'}</text><text x="${x}" y="${base+17}" text-anchor="middle" font-size="11" fill="#526b7d">${month.slice(5)}/${Number(month.slice(0,4))+543}</text><text x="${x}" y="${base+33}" text-anchor="middle" font-size="11" fill="#526b7d">${record?`${record.passed}/${record.total}`:'ไม่มีข้อมูล'}</text>`}).join('');
 return `<div class="kpi-year-chart-scroll mini-cqi-kpi1-chart"><svg class="kpi-exec-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="KPI 1 ผลรายเดือน พร้อมจำนวนแผนผ่านต่อจำนวนออกหน่วยจริง"><rect x="5" y="5" width="${w-10}" height="${h-10}" rx="18" fill="#fff" stroke="#e7eff5"/><text x="${left}" y="33" font-size="17" font-weight="700" fill="#234969">KPI 1 · ออกหน่วยตามผลคาดการณ์รายเดือน</text><text x="${left}" y="55" font-size="12" fill="#536d80">เปอร์เซ็นต์ · ตัวเลขใต้เดือน = ผ่าน / ออกหน่วยจริงที่พบในข้อมูล</text><line x1="${left}" x2="${w-25}" y1="${top}" y2="${top}" stroke="#df6866" stroke-width="2" stroke-dasharray="6 5"/><text x="${w-29}" y="${top-6}" text-anchor="end" font-size="12" fill="#b94646">เป้าหมาย 100%</text><line x1="${left}" x2="${w-25}" y1="${base}" y2="${base}" stroke="#dae6ef"/>${columns}</svg></div>`;
}
...
async function loadMiniCqi(){
  ...
  try{
    ...
    const [allOutings,actualRegistered,dep,families]=await Promise.all([
      MinimumStockBackend.getCqiPlans(compareFrom,compareTo),
      MinimumStockBackend.getCqiOutings(compareFrom,compareTo),
    ...
    const addActual=(date,site,source)=>{if(!/^\d{4}-\d{2}-\d{2}$/.test(String(date||''))||!String(site||'').trim()||!inScope(date))return;const key=miniCqiEventKey(date,site);if(!actualByKey.has(key))actualByKey.set(key,{outing_date:date,site:String(site).trim(),source});else if(source==='ลงทะเบียน')actualByKey.get(key).source='ลงทะเบียน + LIS'};
    (families.rows||[]).filter(row=>row.aggregateEligible!==false&&row.sourceGroup===OUTREACH_GROUP_SELF_OUTREACH).forEach(row=>addActual(String(row.collectDate||row.cohortDate||'').slice(0,10),row.donateSource,'LIS'));
    actualRegistered.forEach(row=>addActual(row.outing_date,row.site,'ลงทะเบียน'));
    const actualOutings=[...actualByKey.values()].sort((a,b)=>a.outing_date.localeCompare(b.outing_date)||a.site.localeCompare(b.site));
    const months=dep.months.filter(row=>inScope(`${row.year}-${String(row.month).padStart(2,'0')}-01`));
    const k1=miniCqiSummary(outings,actualOutings), k2={num:months.reduce((n,m)=>n+miniCqiRoutineCounts(m).num,0),den:months.reduce((n,m)=>n+miniCqiRoutineCounts(m).den,0)};
    ...
    miniCqiResult={range,outings,actualOutings,months,details,counts,k1,k2,k3,productScope:miniCqiProductScope,productGroups:[...miniCqiProductGroups],missingExpiryUsed,years:[...miniCqiYears]};
    ...
    k1.events.forEach(o=>{const month=miniCqiMonth(o.outing_date),entry=k1Monthly.get(month)||{passed:0,total:0,missing:0};entry.total++;if(o.passed)entry.passed++;if(!o.plans.length)entry.missing++;k1Monthly.set(month,entry)});
    ...
    box.innerHTML=`<div class="simple-page-head"><div><h1>ผล CQI · ออกหน่วยบริจาคโลหิต</h1></div><button class="btn btn-light no-print" onclick="exportMiniCqi()">ส่งออก CSV รวมพร้อมสูตร</button></div>
      ...
      <div data-mini-panel="overview" class="simple-kpi-grid blood-kpi-main-grid mb-3">${miniCqiCard('1 · ออกหน่วยตามช่วงคาดการณ์เสี่ยงขาด',k1.passed,k1.total,'100%',k1.passed===k1.total,`${k1.missing.length} หน่วยยังไม่มีแผน · ตรวจความครบกับตารางออกหน่วย`)}${miniCqiCard('2 · กาชาด Routine · เลือดแดง',k2.num,k2.den,'≤ 20%',k2.num/k2.den<=.20,'ตัดเลือดหายาก/จำเป็นตามนิยามหน้า KPI เดิม')}${miniCqiCard('3 · ใช้ประโยชน์เลือดแดงออกหน่วย',k3.num,k3.den,'≥ 95%',k3.num/k3.den>=.95,`${counts['รอติดตามผล']} รอติดตาม · ${counts['ต้องตรวจสอบ']} ต้องตรวจสอบ · ${missingExpiryUsed} ถุงยังยืนยันก่อนหมดอายุไม่ได้`)}</div>
      <div data-mini-panel="kpi1" class="simple-panel mb-3"><div class="panel-heading-row"><h3>KPI 1 · แผนออกหน่วยอ้างอิงผลคาดการณ์เลือดแดง</h3><div class="no-print"><button class="btn btn-light" onclick="exportMiniCqiChart('kpi1')">PNG กราฟ KPI 1</button> <button class="btn btn-light" onclick="exportMiniCqiSection('kpi1')">ส่งออก KPI 1</button></div></div>
        <div class="mini-cqi-hint">เป้าหมาย 100% · บันทึกแผนก่อนวันออกหน่วย</div><details class="mini-cqi-help"><summary>วิธีนับ KPI 1</summary><p>ตัวตั้ง: แผนที่คำนวณครบ 4 หมู่และคาดว่ามีอย่างน้อย 1 หมู่ต่ำกว่า Minimum ณ วันทราบแผน · ตัวหาร: หน่วยที่ออกจริงตามข้อมูล LIS และรายการลงทะเบียนออกหน่วย นับวันและสถานที่ไม่ซ้ำ ตรวจความครบกับตารางออกหน่วยจริง การกรอกวันเวลาย้อนหลังไม่ใช่หลักฐานว่าบันทึกในวันนั้น</p></details>
        ...
        ${miniCqiKpi1PresentationChart(k1Monthly,miniCqiYears,inScope)}${miniCqiKpi1MonthlyTable(k1Monthly,miniCqiYears,inScope)}<div class="mini-cqi-hint">ตัวหารมาจากข้อมูลออกหน่วยใน LIS และรายการลงทะเบียน (วัน + สถานที่ไม่ซ้ำ) · เดือนที่ไม่มีข้อมูลไม่แสดงเป็น 100% · โปรดเทียบตารางออกหน่วยจริงก่อนนำเสนอ</div><details class="mini-cqi-help"><summary>ตรวจรายการออกหน่วยจริง (${k1.events.length})</summary><div class="table-responsive"><table class="table simple-table"><thead><tr><th>วันที่</th><th>สถานที่</th><th>ที่มา</th><th>แผน KPI 1</th><th>ผล</th></tr></thead><tbody>${k1.events.map(event=>`<tr><td>${miniCqiHtml(event.outing_date)}</td><td>${miniCqiHtml(event.site)}</td><td>${miniCqiHtml(event.source)}</td><td>${event.plans.length} รายการ${event.plans.length>1?' · ตรวจรายการซ้ำ':''}</td><td>${event.passed?'ผ่าน':event.plans.length?'ไม่ผ่าน/ประเมินไม่ได้':'ยังไม่มีแผน'}</td></tr>`).join('')||'<tr><td colspan="5">ไม่พบรายการออกหน่วยจริงในข้อมูลที่เลือก</td></tr>'}</tbody></table></div></details><details class="mini-cqi-help"><summary>ที่มาของผลคาดการณ์</summary><p>ใช้ Stock ต้นวันและยอดใช้ก่อนวันทราบแผนจาก LIS เพื่อประกอบการตัดสินใจ</p></details>
...
function exportMiniCqiSection(key){
 ...
 const formulas={kpi1:'ครั้งที่ผล ณ วันที่ทราบแผนคำนวณได้และคาดว่ามีหมู่เลือดแดงต่ำกว่า Minimum / ครั้งออกหน่วยจริงจาก LIS และรายการลงทะเบียนที่ถึงวันออกหน่วยแล้ว; ตรวจความครบกับตารางออกหน่วย ไม่ใช่นาที',kpi2:'จำนวนถุงกาชาด Routine / จำนวนถุง Routine รับเข้าทั้งหมด',kpi3:'ถุงต้นทางที่ใช้/จ่าย/ส่งต่อตามสถานะ LIS / (ถุงที่ใช้/จ่าย/ส่งต่อ + ถุงหมดอายุ)'};
 ...
 if(key==='kpi1'){rows.push([],['เดือน','พบหมู่เสี่ยง','ออกหน่วยจริงที่พบในข้อมูล','อัตรา']);const monthly=new Map();r.k1.events.forEach(o=>{const key=miniCqiMonth(o.outing_date),m=monthly.get(key)||{passed:0,total:0};m.total++;if(o.passed)m.passed++;monthly.set(key,m)});[...monthly].sort((a,b)=>a[0].localeCompare(b[0])).forEach(([month,m])=>rows.push([month,m.passed,m.total,miniCqiPct(m.passed,m.total)]));rows.push([],['วันที่ออกหน่วยจริง','สถานที่','ที่มา','แผน KPI 1','ผล']);r.k1.events.forEach(o=>rows.push([o.outing_date,o.site,o.source,o.plans.length,o.passed?'ผ่าน':o.plans.length?'ไม่ผ่าน/ประเมินไม่ได้':'ยังไม่มีแผน']));rows.push([],['วันที่ออกหน่วย','สถานที่','เวลาทราบแผน','หลักฐาน','เวลาบันทึกจริง','หมู่เสี่ยง','ผล']);r.outings.forEach(o=>rows.push([o.outing_date,o.site,o.decision_at,o.evidence_ref,o.created_at,(o.forecast?.riskGroups||[]).join(', '),miniCqiPlanPassed(o)?'ผ่าน':o.forecast?.canEvaluate?'ไม่พบหมู่เสี่ยง':'ประเมินไม่ได้']))}
...
function exportMiniCqi(){
 const r=miniCqiResult;if(!r)return;
 const lines=[['ผล CQI Mini Mobile Blood Donation',r.range.from,r.range.to,'ปีที่เลือก',r.years.map(y=>y+543).join('; ')],['KPI','ตัวตั้ง','ตัวหาร','ร้อยละ','เป้าหมาย','สูตร'],['KPI 1',r.k1.passed,r.k1.total,miniCqiPct(r.k1.passed,r.k1.total),'100%','ผล ณ วันทราบแผนคาดว่ามีหมู่เลือดแดงต่ำกว่า Minimum / ครั้งออกหน่วยจริงจาก LIS และรายการลงทะเบียนที่ถึงวันออกหน่วยแล้ว; ตรวจความครบกับตารางออกหน่วย'],['KPI 2 · เลือดแดง',r.k2.num,r.k2.den,miniCqiPct(r.k2.num,r.k2.den),'≤20%','กาชาด Routine / รับเข้า Routine ทั้งหมด; ไม่รวม Rare'],['KPI 3 · '+(r.productGroups.join('; ')),r.k3.num,r.k3.den,miniCqiPct(r.k3.num,r.k3.den),'≥95%','ใช้/จ่าย/ส่งต่อตาม LIS / (ใช้/จ่าย/ส่งต่อ + หมดอายุ); นับถุงต้นทาง; ถุงที่ขาดวันหมดอายุยังไม่ยืนยันว่าใช้ก่อนหมดอายุ'],['KPI 3 ถุงที่ใช้แต่ไม่มีวันหมดอายุ',r.missingExpiryUsed],[],['เดือน','กาชาด Routine','Routine รับเข้าทั้งหมด','อัตรา']];
