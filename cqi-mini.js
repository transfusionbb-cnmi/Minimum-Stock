...
function miniCqiKpi1PresentationChart(monthly,years,inScope){
 ...
 const w=Math.max(820,months.length*190+140),h=345,left=90,right=75,top=137,base=269,step=(w-left-right)/Math.max(1,months.length-1),xAt=i=>months.length===1?(left+w-right)/2:left+i*step,yAt=rate=>base-(base-top)*rate/100;
 const total=months.reduce((n,ym)=>n+monthly.get(ym).total,0),passed=months.reduce((n,ym)=>n+monthly.get(ym).passed,0);
 const grid=[0,50,100].map(rate=>`<line x1="${left}" x2="${w-right}" y1="${yAt(rate)}" y2="${yAt(rate)}" stroke="${rate===100?'#da6f6d':'#e1ebf2'}" ${rate===100?'stroke-dasharray="6 5"':''}/><text x="${left-12}" y="${yAt(rate)+5}" text-anchor="end" font-size="12" fill="#526b7d">${rate}%</text>`).join('');
 const segments=months.slice(1).map((month,i)=>{const prev=months[i],a=monthly.get(prev),b=monthly.get(month),gap=(Number(month.slice(0,4))-Number(prev.slice(0,4)))*12+Number(month.slice(5))-Number(prev.slice(5));return `<line x1="${xAt(i)}" y1="${yAt(100*a.passed/a.total)}" x2="${xAt(i+1)}" y2="${yAt(100*b.passed/b.total)}" stroke="#357fb6" stroke-width="3" ${gap>1?'stroke-dasharray="7 6"':''}/>`}).join('');
 const dots=months.map((ym,i)=>{const item=monthly.get(ym),rate=100*item.passed/item.total,x=xAt(i),y=yAt(rate),label=`${String(Number(ym.slice(5))).padStart(2,'0')}/${Number(ym.slice(0,4))+543}`;return `<circle cx="${x}" cy="${y}" r="8" fill="#fff" stroke="${rate===100?'#278f79':'#df8b42'}" stroke-width="4"/><text x="${x}" y="${Math.max(118,y-16)}" text-anchor="middle" font-size="17" font-weight="700" fill="#264963">${rate.toFixed(1)}%</text><text x="${x}" y="${Math.min(base-15,y+30)}" text-anchor="middle" font-size="13" font-weight="700" fill="#34566a">${item.passed}/${item.total} แผนออกหน่วย</text><text x="${x}" y="${base+25}" text-anchor="middle" font-size="13" fill="#536d80">${label}</text>`}).join('');
 return `<div class="mini-cqi-chart-toolbar no-print"><button type="button" class="btn btn-light" onclick="exportMiniCqiChart('kpi1')">ดาวน์โหลดกราฟรายเดือน PNG</button></div><div class="kpi-year-chart-scroll mini-cqi-kpi1-chart"><svg class="kpi-exec-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="KPI 1 กราฟเส้นแสดงเปอร์เซ็นต์และจำนวนแผนออกหน่วยที่พบความเสี่ยงแต่ละเดือน"><rect x="5" y="5" width="${w-10}" height="${h-10}" rx="18" fill="#fff" stroke="#e7eff5"/><text x="${left}" y="34" font-size="19" font-weight="700" fill="#234969">KPI 1 · แผนออกหน่วยที่พบความเสี่ยงขาดเลือด</text><text x="${left}" y="59" font-size="13" fill="#526b7d">ร้อยละของแผนที่คาดว่ามีเลือดแดงต่ำกว่า Minimum · เป้าหมาย 100%</text><text x="${left}" y="85" font-size="14" font-weight="700" fill="#278f79">รวม ${passed}/${total} แผนออกหน่วย · ${months.length} เดือนที่มีแผน</text><text x="${w-right}" y="${top-9}" text-anchor="end" font-size="12" fill="#bd5656">เกณฑ์ 100%</text>${grid}${segments}${dots}<text x="${left}" y="${h-18}" font-size="11" fill="#657a8b">เส้นประระหว่างจุด = ช่วงเดือนที่ไม่มีแผนบันทึก · ไม่นับเป็น 0% หรือ 100%</text></svg></div>`;
}
...
function exportMiniCqiTablePng(key){
 ...
 img.onload=()=>{try{const canvas=document.createElement('canvas');canvas.width=w*2;canvas.height=h*2;const ctx=canvas.getContext('2d');ctx.scale(2,2);ctx.drawImage(img,0,0);const link=document.createElement('a');link.download=`CQI-${key}-table-${r.range.from}-${r.range.to}.png`;link.href=canvas.toDataURL('image/png');link.click()}catch(err){showModal('error','ส่งออกภาพตารางไม่สำเร็จ',err.message)}finally{URL.revokeObjectURL(url)}};
