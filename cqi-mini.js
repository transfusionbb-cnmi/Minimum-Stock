  ...
  try{
    ...
    box.innerHTML=`<div class="simple-page-head"><div><h1>ผล CQI: Mini Mobile Blood Donation</h1><div class="small-muted">แสดงข้อมูลตามช่วงเดือนที่รับเข้าหรือวันที่ออกหน่วย</div></div><button class="btn btn-light no-print" onclick="exportMiniCqi()">ส่งออก CSV รวมพร้อมสูตร</button></div>
      ...
      <div data-mini-panel="kpi1" class="simple-panel mb-3"><div class="panel-heading-row"><h3>KPI 1 · แผนออกหน่วยอ้างอิงผลคาดการณ์เลือดแดง</h3><div class="no-print"><button class="btn btn-light" onclick="exportMiniCqiChart('kpi1')">PNG กราฟนี้</button> <button class="btn btn-light" onclick="exportMiniCqiSection('kpi1')">ส่งออก KPI 1</button></div></div>
        <div class="small-muted">เป้าหมาย 100% · ตัวตั้ง: ครั้งออกหน่วยที่คำนวณได้ครบ 4 หมู่และคาดว่ามีอย่างน้อยหนึ่งหมู่ต่ำกว่า Minimum ณ วันทราบแผน / ตัวหาร: ครั้งออกหน่วยที่ลงทะเบียนและถึงวันออกหน่วยแล้ว · ตรวจความครบกับตารางออกหน่วยจริง · การกรอกวันที่เวลาย้อนหลังไม่ใช่หลักฐานว่าบันทึกไว้ในวันนั้น</div>
        <form class="mini-cqi-form-grid no-print" onsubmit="registerMiniCqiPlan(event)"><label>วันที่–เวลาที่ทราบว่าจะออกหน่วย (เวลาไทยตามหลักฐาน)<input name="decision_at" type="datetime-local" class="form-control" required></label><label>หลักฐาน / เลขกิจกรรม Staff Planner / หนังสือ<input name="evidence_ref" class="form-control" required></label><label>วันที่ออกหน่วย<input name="outing_date" type="date" class="form-control" required></label><label>สถานที่<input name="site" class="form-control" required></label><button class="btn btn-main" type="submit">ตรวจผลคาดการณ์และบันทึกแผน</button></form>
        ${renderKpiYearOverlayChart(k1Chart,'value',{title:'แผนออกหน่วยในช่วงเสี่ยงรายเดือน (%)',unit:'%',yMax:100,target:100})}<div class="small-muted">ใช้ Stock ต้นวันและยอดใช้ก่อนวันที่ทราบแผนเท่านั้น; LIS ระบุการเคลื่อนไหวระดับวัน จึงยืนยัน Stock ณ นาทีที่ทราบแผนย้อนหลังไม่ได้ · ผลกรอกภายหลังติดป้าย “คำนวณย้อนหลัง” · ไม่ปรับผลตาม Stock วันที่ใกล้ออกหน่วย</div>
        <div class="table-responsive"><table class="table simple-table"><thead><tr><th>ออกหน่วย</th><th>สถานที่</th><th>เวลาทราบแผน</th><th>หลักฐานอ้างอิง</th><th>ชนิดผล</th><th>หมู่เสี่ยง</th><th>ผล</th></tr></thead><tbody>${outings.map(row=>`<tr><td>${miniCqiHtml(row.outing_date)}</td><td>${miniCqiHtml(row.site)}</td><td>${miniCqiHtml(new Date(row.decision_at).toLocaleString('th-TH',{timeZone:'Asia/Bangkok'}))}</td><td>${miniCqiHtml(row.evidence_ref)}</td><td>${row.evidence_kind==='reconstructed'?'คำนวณย้อนหลัง':'บันทึกขณะทราบแผน'}</td><td>${miniCqiHtml((row.forecast?.riskGroups||[]).join(', ')||'ไม่พบ')}</td><td>${miniCqiPlanPassed(row)?'ผ่าน · พบหมู่เสี่ยง':row.forecast?.canEvaluate?'ไม่พบหมู่เสี่ยง':'ประวัติไม่ครบ · ประเมินไม่ได้'}</td></tr>`).join('')||'<tr><td colspan="7">ยังไม่มีแผนตามเวลาทราบแผนในช่วงนี้ · แผน KPI 1 รุ่นวันออกหนังสือเก่าไม่ถูกย้ายมาเป็นหลักฐานเวลาโดยอัตโนมัติ</td></tr>'}</tbody></table></div></div>
...
async function registerMiniCqiPlan(event){
 ...
 try{const plan=await MinimumStockBackend.registerCqiPlan({decisionAt:decisionTime.toISOString(),evidenceRef:String(f.get('evidence_ref')||'').trim(),outingDate,site:String(f.get('site')||'').trim()});
 miniCqiActiveTab='kpi1';await loadMiniCqi();showModal('success','บันทึกแผนแล้ว',plan.evidence_kind==='reconstructed'?'ผลคำนวณย้อนหลัง กรุณาเทียบหลักฐานเวลาที่ทราบแผน':'บันทึกผลขณะทราบแผนแล้ว');
