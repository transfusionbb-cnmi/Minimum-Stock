...
function miniCqiYearChoices(from,to){let choices='';for(let year=Number(from.slice(0,4));year<=Number(to.slice(0,4));year++){choices+=`<label><input type="checkbox" name="miniCqiYear" value="${year}" ${miniCqiYears.includes(year)?'checked':''}> ${year+543}</label>`}return choices}
...
async function loadMiniCqi(){
  ...
  if(!miniCqiYears.length)miniCqiYears=Array.from({length:Math.min(2,Number(range.to.slice(0,4))-Number(range.from.slice(0,4))+1)},(_,i)=>Number(range.to.slice(0,4))-i).sort();
  ...
  try{
    ...
    box.innerHTML=`<div class="simple-page-head"><div><h1>ผล CQI: Mini Mobile Blood Donation</h1><div class="small-muted">แสดงข้อมูลตามช่วงเดือนที่รับเข้าหรือวันที่ออกหน่วย</div></div><button class="btn btn-light no-print" onclick="exportMiniCqi()">ส่งออก CSV รวมพร้อมสูตร</button></div>
      ...
      <div data-mini-panel="kpi1" class="simple-panel mb-3"><div class="panel-heading-row"><h3>KPI 1 · แผนออกหน่วยอ้างอิงผลคาดการณ์เลือดแดง</h3><div class="no-print"><button class="btn btn-light" onclick="exportMiniCqiChart('kpi1')">PNG กราฟนี้</button> <button class="btn btn-light" onclick="exportMiniCqiSection('kpi1')">ส่งออก KPI 1</button></div></div>
        <div class="small-muted">เป้าหมาย 100% · ตัวตั้ง: ครั้งออกหน่วยที่คำนวณได้ครบ 4 หมู่และพบอย่างน้อยหนึ่งหมู่เสี่ยงต่ำกว่า Minimum ณ วันที่ออกหนังสือ / ตัวหาร: ครั้งออกหน่วยที่ลงทะเบียนและถึงวันออกหน่วยแล้ว · ต้องเทียบทะเบียนนี้กับตารางออกหน่วยจริง · ผลคำนวณย้อนหลังมีป้ายแยก</div>
        <form class="mini-cqi-form-grid no-print" onsubmit="registerMiniCqiPlan(event)"><label>วันที่ออกหนังสือ<input name="letter_date" type="date" class="form-control" max="${getTodayYmd()}" required></label><label>เลขที่หนังสือ / หลักฐาน<input name="letter_ref" class="form-control" required></label><label>วันที่ออกหน่วย<input name="outing_date" type="date" class="form-control" required></label><label>สถานที่<input name="site" class="form-control" required></label><button class="btn btn-main" type="submit">ตรวจผลคาดการณ์และบันทึกแผน</button></form>
        ${renderKpiYearOverlayChart(k1Chart,'value',{title:'แผนออกหน่วยในช่วงเสี่ยงรายเดือน (%)',unit:'%',yMax:100,target:100})}<div class="small-muted">ใช้เฉพาะยอดรับเข้าและประวัติใช้ก่อนวันออกหนังสือ · วันออกหนังสือเก่าที่กรอกวันนี้ติดป้าย “คำนวณย้อนหลัง” · หมู่เสี่ยงเปลี่ยนได้เมื่อ Stock เปลี่ยน แต่ผลที่บันทึกไว้ไม่เปลี่ยน</div>
        <div class="table-responsive"><table class="table simple-table"><thead><tr><th>ออกหน่วย</th><th>สถานที่</th><th>วันออกหนังสือ</th><th>เลขที่หนังสือ</th><th>หลักฐาน</th><th>หมู่เสี่ยง</th><th>ผล</th></tr></thead><tbody>${outings.map(row=>`<tr><td>${miniCqiHtml(row.outing_date)}</td><td>${miniCqiHtml(row.site)}</td><td>${miniCqiHtml(row.letter_date)}</td><td>${miniCqiHtml(row.letter_ref)}</td><td>${row.evidence_kind==='reconstructed'?'คำนวณย้อนหลัง':'บันทึกในวันออกหนังสือ'}</td><td>${miniCqiHtml((row.forecast?.riskGroups||[]).join(', ')||'ไม่พบ')}</td><td>${miniCqiPlanPassed(row)?'ผ่าน · พบหมู่เสี่ยง':row.forecast?.canEvaluate?'ไม่พบหมู่เสี่ยง':'ประวัติไม่ครบ · ประเมินไม่ได้'}</td></tr>`).join('')||'<tr><td colspan="7">ยังไม่มีแผนที่ลงทะเบียนในช่วงนี้ · กรอกวันที่หนังสือและวันออกหน่วยด้านบน</td></tr>'}</tbody></table></div></div>
      <div data-mini-panel="kpi2" class="simple-panel mb-3"><div class="panel-heading-row"><h3>KPI 2 · พึ่งกาชาด Routine · เลือดแดง</h3><div class="no-print"><button class="btn btn-light" onclick="exportMiniCqiChart('kpi2')">PNG กราฟนี้</button> <button class="btn btn-light" onclick="exportMiniCqiSection('kpi2')">ส่งออก KPI 2</button></div></div><div class="small-muted">ตัวตั้ง ${k2.num} / ตัวหาร ${k2.den} ถุง · นับเฉพาะเลือดแดง Routine ตามหน้า KPI เดิม · สรุปรวมจากจำนวนถุง ไม่เฉลี่ยเปอร์เซ็นต์รายเดือน</div>${renderKpiYearOverlayChart(comparison,'value',{title:'เทียบเดือนเดียวกันตามปีที่เลือก · Routine (%)',unit:'%',yMax:100,target:20})}<div class="table-responsive"><table class="table simple-table"><thead><tr><th>เดือน</th><th>กาชาด Routine</th><th>Routine รับเข้าทั้งหมด</th><th>อัตรา</th></tr></thead><tbody>${months.map(monthRow).join('')}</tbody></table></div></div>
      <div data-mini-panel="kpi3" class="simple-panel"><div class="panel-heading-row"><h3>KPI 3 · ใช้ประโยชน์เลือดออกหน่วย · ${miniCqiProductGroups.map(x=>({RBC:'เลือดแดง',PLASMA:'พลาสมา',PLATELET:'เกล็ดเลือด',CRYO:'Cryo'}[x])).join(', ')}</h3><div class="no-print"><button class="btn btn-light" onclick="exportMiniCqiChart('kpi3')">PNG กราฟนี้</button> <button class="btn btn-light" onclick="exportMiniCqiSection('kpi3')">ส่งออก KPI 3</button></div></div>${renderKpiYearOverlayChart(k3Chart,'value',{title:'อัตราใช้ประโยชน์เลือดออกหน่วยรายเดือน (%)',unit:'%',yMax:100,target:95})}<div class="mini-cqi-counts">${Object.entries(counts).map(([key,count])=>`<span>${miniCqiHtml(key)} <b>${count}</b></span>`).join('')}</div><div class="small-muted">ตัวตั้ง ${k3.num} / ตัวหาร ${k3.den} · Rejected, รอติดตาม และต้องตรวจสอบ ไม่อยู่ในตัวหาร · ${missingExpiryUsed} ถุงใช้แล้วแต่ยังยืนยันวันหมดอายุไม่ได้; ตัวเลขนี้เป็นอัตราการใช้ประโยชน์ตามสถานะ LIS</div><div class="table-responsive"><table class="table simple-table"><thead><tr><th>เดือนรับเข้า</th><th>ใช้</th><th>หมดอายุ</th><th>Rejected</th><th>รอติดตาม</th><th>ต้องตรวจสอบ</th><th>ผล</th></tr></thead><tbody>${[...monthlyBagCounts].sort((a,b)=>a[0].localeCompare(b[0])).map(([month,c])=>`<tr><td>${miniCqiHtml(month)}</td><td>${c.used}</td><td>${c.expired}</td><td>${c.rejected}</td><td>${c.pending}</td><td>${c.review}</td><td>${miniCqiPct(c.used,c.used+c.expired)}</td></tr>`).join('')}</tbody></table></div><details><summary>ดูรายถุง (${details.length.toLocaleString()} ถุง)</summary><div class="table-responsive mini-cqi-bag-scroll"><table class="table simple-table"><thead><tr><th>Bag No.</th><th>รับเข้า</th><th>จุดออกหน่วย</th><th>ผลิตภัณฑ์</th><th>ผล</th><th>เหตุผลที่ต้องตรวจสอบ</th></tr></thead><tbody>${details.map(row=>`<tr><td>${miniCqiHtml(row.bagNumber)}</td><td>${miniCqiHtml(row.cohortDate)}</td><td>${miniCqiHtml(row.site)}</td><td>${miniCqiHtml(row.productType)}</td><td>${miniCqiHtml(row.status)}</td><td>${miniCqiHtml(row.reason)}</td></tr>`).join('')}</tbody></table></div></details></div>`;
    miniCqiOpenTab(miniCqiActiveTab);
  }catch(err){box.innerHTML=`<div class="simple-panel"><h3>เปิดผล CQI ไม่ได้</h3><p>${miniCqiHtml(err.message)}</p><small>ตรวจว่าได้รัน SQL-v2.9.79-CQI-PLAN-SNAPSHOT.sql แล้ว</small></div>`;}
}
...
function exportMiniCqiSection(key){
 ...
 const formulas={kpi1:'ครั้งที่ผล ณ วันออกหนังสือคำนวณได้และคาดว่ามีหมู่เลือดแดงต่ำกว่า Minimum / ครั้งออกหน่วยที่ลงทะเบียนและถึงวันออกหน่วยแล้ว',kpi2:'จำนวนถุงกาชาด Routine / จำนวนถุง Routine รับเข้าทั้งหมด',kpi3:'ถุงต้นทางที่ใช้/จ่าย/ส่งต่อตามสถานะ LIS / (ถุงที่ใช้/จ่าย/ส่งต่อ + ถุงหมดอายุ)'};
 ...
 if(key==='kpi1'){rows.push([],['วันที่ออกหน่วย','สถานที่','วันที่ออกหนังสือ','เลขที่หนังสือ','ชนิดหลักฐาน','หมู่เสี่ยง','ผล']);r.outings.forEach(o=>rows.push([o.outing_date,o.site,o.letter_date,o.letter_ref,o.evidence_kind==='reconstructed'?'คำนวณย้อนหลัง':'บันทึกในวันออกหนังสือ',(o.forecast?.riskGroups||[]).join(', '),miniCqiPlanPassed(o)?'ผ่าน':o.forecast?.canEvaluate?'ไม่พบหมู่เสี่ยง':'ประเมินไม่ได้']))}
...
function exportMiniCqi(){
 const r=miniCqiResult;if(!r)return;
 const lines=[['ผล CQI Mini Mobile Blood Donation',r.range.from,r.range.to,'ปีที่เลือก',r.years.map(y=>y+543).join('; ')],['KPI','ตัวตั้ง','ตัวหาร','ร้อยละ','เป้าหมาย','สูตร'],['KPI 1',r.k1.passed,r.k1.total,miniCqiPct(r.k1.passed,r.k1.total),'100%','ผล ณ วันออกหนังสือคาดว่ามีหมู่เลือดแดงต่ำกว่า Minimum / ครั้งออกหน่วยที่ลงทะเบียนและถึงวันออกหน่วยแล้ว'],['KPI 2 · เลือดแดง',r.k2.num,r.k2.den,miniCqiPct(r.k2.num,r.k2.den),'≤20%','กาชาด Routine / รับเข้า Routine ทั้งหมด; ไม่รวม Rare'],['KPI 3 · '+(r.productGroups.join('; ')),r.k3.num,r.k3.den,miniCqiPct(r.k3.num,r.k3.den),'≥95%','ใช้/จ่าย/ส่งต่อตาม LIS / (ใช้/จ่าย/ส่งต่อ + หมดอายุ); นับถุงต้นทาง; ถุงที่ขาดวันหมดอายุยังไม่ยืนยันว่าใช้ก่อนหมดอายุ'],['KPI 3 ถุงที่ใช้แต่ไม่มีวันหมดอายุ',r.missingExpiryUsed],[],['เดือน','กาชาด Routine','Routine รับเข้าทั้งหมด','อัตรา']];
 ...
 lines.push([],['วันออกหน่วย','สถานที่','วันออกหนังสือ','เลขที่หนังสือ','หลักฐาน','หมู่เสี่ยง','ผล']);
 r.outings.forEach(o=>lines.push([o.outing_date,o.site,o.letter_date,o.letter_ref,o.evidence_kind==='reconstructed'?'คำนวณย้อนหลัง':'บันทึกในวันออกหนังสือ',(o.forecast?.riskGroups||[]).join(', '),miniCqiPlanPassed(o)?'ผ่าน':o.forecast?.canEvaluate?'ไม่พบหมู่เสี่ยง':'ประเมินไม่ได้']));
...
async function registerMiniCqiPlan(event){
 event.preventDefault();const form=event.currentTarget,f=new FormData(form);
 const letterDate=String(f.get('letter_date')||''),outingDate=String(f.get('outing_date')||'');
 if(letterDate>getTodayYmd()||outingDate<=letterDate)return showModal('error','ตรวจวันที่อีกครั้ง','วันหนังสือต้องไม่เกินวันนี้ และวันออกหน่วยต้องหลังวันหนังสือ');
 const button=form.querySelector('[type=submit]');button.disabled=true;
 try{const plan=await MinimumStockBackend.registerCqiPlan({letterDate,letterRef:String(f.get('letter_ref')||'').trim(),outingDate,site:String(f.get('site')||'').trim()});
 miniCqiActiveTab='kpi1';await loadMiniCqi();showModal('success','บันทึกแผนแล้ว',plan.evidence_kind==='reconstructed'?'ผลคำนวณย้อนหลัง แสดงแยกจากผลที่บันทึกในวันออกหนังสือ':'บันทึกผล ณ วันออกหนังสือแล้ว');
