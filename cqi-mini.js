...
function miniCqiKpi1MonthlyTable(monthly,years,inScope){
 ...
 return `<details class="mini-cqi-help mini-cqi-coverage" open><summary>ผลรายเดือน (${rows.length} เดือน)</summary><div class="table-responsive"><table class="table simple-table"><thead><tr><th>เดือนตามแผน</th><th>พบหมู่เสี่ยง</th><th>แผนที่บันทึก</th><th>อัตรา</th></tr></thead><tbody>${rows.map(month=>{const row=monthly.get(month),year=Number(month.slice(0,4))+543;return `<tr><td>${month.slice(5)}/${year}</td><td>${row?row.passed:'—'}</td><td>${row?row.total:'—'}</td><td><b>${row?miniCqiPct(row.passed,row.total):'ยังไม่มีแผน'}</b></td></tr>`}).join('')}</tbody></table></div></details>`;
}
function miniCqiKpi1PresentationChart(monthly,years,inScope){
 ...
 const columns=months.map((month,i)=>{const record=monthly.get(month),x=left+i*step+step/2,percent=record?record.passed/record.total*100:0,height=(base-top)*percent/100,color=record?(record.passed===record.total?'#278f79':'#dd8b3c'):'#dce7ee';return `<rect x="${x-17}" y="${record?base-height:base-5}" width="34" height="${record?Math.max(height,2):5}" rx="4" fill="${color}"/><text x="${x}" y="${record?Math.max(91,base-height-13):base-17}" text-anchor="middle" font-size="12" font-weight="700" fill="${record?'#264963':'#60798c'}">${record?miniCqiPct(record.passed,record.total):'—'}</text><text x="${x}" y="${base+17}" text-anchor="middle" font-size="11" fill="#526b7d">${month.slice(5)}/${Number(month.slice(0,4))+543}</text><text x="${x}" y="${base+33}" text-anchor="middle" font-size="11" fill="#526b7d">${record?`${record.passed}/${record.total}`:'ไม่มีแผน'}</text>`}).join('');
 return `<div class="kpi-year-chart-scroll mini-cqi-kpi1-chart"><svg class="kpi-exec-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="KPI 1 ผลรายเดือน พร้อมจำนวนแผนผ่านต่อจำนวนแผนที่บันทึก"><rect x="5" y="5" width="${w-10}" height="${h-10}" rx="18" fill="#fff" stroke="#e7eff5"/><text x="${left}" y="33" font-size="17" font-weight="700" fill="#234969">KPI 1 · แผนที่พบความเสี่ยงรายเดือน</text><text x="${left}" y="55" font-size="12" fill="#536d80">เปอร์เซ็นต์ · ตัวเลขใต้เดือน = แผนผ่าน / แผนที่บันทึก</text><line x1="${left}" x2="${w-25}" y1="${top}" y2="${top}" stroke="#df6866" stroke-width="2" stroke-dasharray="6 5"/><text x="${w-29}" y="${top-6}" text-anchor="end" font-size="12" fill="#b94646">เป้าหมาย 100%</text><line x1="${left}" x2="${w-25}" y1="${base}" y2="${base}" stroke="#dae6ef"/>${columns}</svg></div>`;
}
