...
function miniCqiKpi1MonthlyTable(monthly,years,inScope){
 ...
 const table=items=>`<div class="table-responsive"><table class="table simple-table"><thead><tr><th>เดือนออกหน่วย</th><th>พบหมู่เสี่ยง</th><th>แผนที่บันทึก</th><th>ผล</th></tr></thead><tbody>${items.map(month=>{const row=monthly.get(month),year=Number(month.slice(0,4))+543;return `<tr><td>${month.slice(5)}/${year}</td><td>${row?row.passed:'—'}</td><td>${row?row.total:'—'}</td><td><b>${row?miniCqiPct(row.passed,row.total):'ไม่มีแผนบันทึก'}</b></td></tr>`}).join('')}</tbody></table></div>`;
 return `<div class="mini-cqi-hint">บันทึกแผน ${recorded.length} จาก ${months.length} เดือนที่เลือก · เดือนที่ไม่มีแผนไม่คิดเป็น 0% หรือ 100%</div>${recorded.length?table(recorded):''}<details class="mini-cqi-help mini-cqi-coverage"><summary>ดูครบทุกเดือน (${months.length} เดือน)</summary>${table(months)}</details>`;
}
function miniCqiKpi1PresentationChart(monthly,years,inScope){
 ...
 const w=Math.max(760,months.length*160+150),h=310,left=85,right=48,top=99,base=245,step=(w-left-right)/months.length;
 const bars=months.map((month,i)=>{const row=monthly.get(month),rate=row.passed/row.total*100,x=left+step*(i+.5),height=(base-top)*rate/100,color=rate===100?'#278f79':'#dd8b3c';return `<rect x="${x-34}" y="${base-height}" width="68" height="${Math.max(2,height)}" rx="7" fill="${color}"/><text x="${x}" y="${Math.max(top-8,base-height-13)}" text-anchor="middle" font-size="16" font-weight="700" fill="#254b65">${miniCqiPct(row.passed,row.total)}</text><text x="${x}" y="${base+24}" text-anchor="middle" font-size="13" font-weight="700" fill="#34566a">${month.slice(5)}/${Number(month.slice(0,4))+543}</text><text x="${x}" y="${base+44}" text-anchor="middle" font-size="12" fill="#536d80">${row.passed}/${row.total} แผน</text>`}).join('');
 return `<div class="mini-cqi-chart-toolbar no-print"><button type="button" class="btn btn-light" onclick="exportMiniCqiChart('kpi1')">ดาวน์โหลดกราฟรายเดือน PNG</button></div><div class="kpi-year-chart-scroll mini-cqi-kpi1-chart"><svg class="kpi-exec-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="KPI 1 อัตราแผนที่พบความเสี่ยงเฉพาะเดือนที่บันทึกแผน"><rect x="5" y="5" width="${w-10}" height="${h-10}" rx="18" fill="#fff" stroke="#e7eff5"/><text x="${left}" y="34" font-size="18" font-weight="700" fill="#234969">KPI 1 · แผนที่พบความเสี่ยง</text><text x="${left}" y="58" font-size="12" fill="#536d80">แสดงเฉพาะเดือนที่บันทึกแผน · ตัวเลข = แผนผ่าน / แผนทั้งหมด</text><line x1="${left}" x2="${w-right}" y1="${top}" y2="${top}" stroke="#df6866" stroke-width="2" stroke-dasharray="6 5"/><text x="${w-right}" y="${top-8}" text-anchor="end" font-size="12" fill="#b94646">เป้าหมาย 100%</text><line x1="${left}" x2="${w-right}" y1="${base}" y2="${base}" stroke="#dae6ef"/>${bars}</svg></div>`;
}
...
async function loadMiniCqi(){
  ...
  try{
    ...
    miniCqiResult={range,outings,months,details,counts,k1,k2,k3,kpi2Years,productScope:miniCqiProductScope,productGroups:[...miniCqiProductGroups],missingExpiryUsed,years:[...miniCqiYears]};
    ...
    box.innerHTML=`<div class="simple-page-head"><div><h1>ผล CQI · ออกหน่วยบริจาคโลหิต</h1></div><button class="btn btn-light no-print" onclick="exportMiniCqi()">ส่งออก CSV รวมพร้อมสูตร</button></div>
      ...
      <div data-mini-panel="overview" class="simple-kpi-grid blood-kpi-main-grid mb-3">${miniCqiCard('1 · ออกหน่วยตามช่วงคาดการณ์เสี่ยงขาด',k1.passed,k1.total,'100%',k1.passed===k1.total,`${k1.missing.length} แผนข้อมูลไม่พอ · ${k1.risk.length} แผนพบหมู่เสี่ยง`)}${miniCqiCard('2 · กาชาด Routine · เลือดแดง',k2.num,k2.den,'≤ 20%',k2.num/k2.den<=.20,'ตัดเลือดหายาก/จำเป็นตามนิยามหน้า KPI เดิม')}${miniCqiCard('3 · ใช้ประโยชน์เลือดแดงออกหน่วย',k3.num,k3.den,'≥ 95%',k3.num/k3.den>=.95,`${counts['รอติดตามผล']} รอติดตาม · ${counts['ต้องตรวจสอบ']} ต้องตรวจสอบ · ${missingExpiryUsed} ถุงยังยืนยันก่อนหมดอายุไม่ได้`)}</div>
      <div data-mini-panel="kpi1" class="simple-panel mb-3"><div class="panel-heading-row"><h3>KPI 1 · แผนออกหน่วยอ้างอิงผลคาดการณ์เลือดแดง</h3><div class="no-print"><button class="btn btn-light" onclick="exportMiniCqiSection('kpi1')">ส่งออก KPI 1</button></div></div>
      ...
      <div data-mini-panel="kpi2" class="simple-panel mb-3"><div class="panel-heading-row"><h3>KPI 2 · พึ่งกาชาด Routine · เลือดแดง</h3><div class="no-print"><button class="btn btn-light" onclick="exportMiniCqiChart('kpi2')">PNG รายปี</button> <button class="btn btn-light" onclick="exportMiniCqiSection('kpi2')">ส่งออก KPI 2</button></div></div><div class="mini-cqi-hint">ช่วงที่เลือก ${k2.num} / ${k2.den} ถุง · เป้าหมาย ≤ 20%</div>${miniCqiKpi2AnnualChart(kpi2Years)}<div class="mini-cqi-hint">แต่ละปีรวมจำนวนถุงก่อนหาร · เทียบการเปลี่ยนแปลงเมื่อมีข้อมูลเดือนเดียวกันทั้งสองปี · ปีที่ข้อมูลไม่ครบ 12 เดือนยังสรุปผลทั้งปีไม่ได้</div><div class="table-responsive"><table class="table simple-table"><thead><tr><th>ปี</th><th>ข้อมูล</th><th>กาชาด Routine</th><th>รับเข้า Routine</th><th>อัตรา</th><th>เทียบปีก่อน*</th></tr></thead><tbody>${annualRows||'<tr><td colspan="6">ยังไม่มีข้อมูล</td></tr>'}</tbody></table></div><details class="mini-cqi-help"><summary>ดูรายละเอียดรายเดือนและวิธีนับ</summary><p>* เทียบเฉพาะปีติดกันที่มีข้อมูลเดือนตรงกัน · นับเฉพาะเลือดแดง Routine ตามหน้า KPI เดิม</p>${renderKpiYearOverlayChart(comparison,'value',{title:'เทียบเดือนเดียวกันตามปีที่เลือก · Routine (%)',unit:'%',yMax:100,target:20})}<div class="table-responsive"><table class="table simple-table"><thead><tr><th>เดือน</th><th>กาชาด Routine</th><th>Routine รับเข้าทั้งหมด</th><th>อัตรา</th></tr></thead><tbody>${months.map(monthRow).join('')}</tbody></table></div></details></div>
...
function exportMiniCqiSection(key){
 ...
 const formulas={kpi1:'ครั้งที่ผล ณ วันที่ทราบแผนคำนวณได้และคาดว่ามีหมู่เลือดแดงต่ำกว่า Minimum / แผนที่บันทึกใน KPI 1 (วัน + สถานที่ไม่ซ้ำ) รวมแผนที่ยังไม่ถึงวันออกหน่วย ไม่ใช่นาที',kpi2:'จำนวนถุงกาชาด Routine / จำนวนถุง Routine รับเข้าทั้งหมด',kpi3:'ถุงต้นทางที่ใช้/จ่าย/ส่งต่อตามสถานะ LIS / (ถุงที่ใช้/จ่าย/ส่งต่อ + ถุงหมดอายุ)'};
 ...
 if(key==='kpi2'){rows.push([],['ปี','เดือนที่มีข้อมูล','กาชาด Routine','Routine รับเข้าทั้งหมด','ร้อยละ']);r.kpi2Years.forEach(y=>rows.push([y.year+543,y.months.join('; '),y.num,y.den,miniCqiPct(y.num,y.den)]));rows.push([],['เดือนที่เลือก','กาชาด Routine','Routine รับเข้าทั้งหมด','ร้อยละ']);r.months.forEach(m=>rows.push([`${m.year}-${m.month}`,miniCqiRoutineCounts(m).num,miniCqiRoutineCounts(m).den,miniCqiPct(miniCqiRoutineCounts(m).num,miniCqiRoutineCounts(m).den)]))}
...
function exportMiniCqi(){
 const r=miniCqiResult;if(!r)return;
 const lines=[['ผล CQI Mini Mobile Blood Donation',r.range.from,r.range.to,'ปีที่เลือก',r.years.map(y=>y+543).join('; ')],['KPI','ตัวตั้ง','ตัวหาร','ร้อยละ','เป้าหมาย','สูตร'],['KPI 1',r.k1.passed,r.k1.total,miniCqiPct(r.k1.passed,r.k1.total),'100%','ผล ณ วันทราบแผนคาดว่ามีหมู่เลือดแดงต่ำกว่า Minimum / แผนที่บันทึกใน KPI 1 (วัน + สถานที่ไม่ซ้ำ) รวมแผนที่ยังไม่ถึงวันออกหน่วย'],['KPI 2 · เลือดแดง',r.k2.num,r.k2.den,miniCqiPct(r.k2.num,r.k2.den),'≤20%','กาชาด Routine / รับเข้า Routine ทั้งหมด; ไม่รวม Rare'],['KPI 3 · '+(r.productGroups.join('; ')),r.k3.num,r.k3.den,miniCqiPct(r.k3.num,r.k3.den),'≥95%','ใช้/จ่าย/ส่งต่อตาม LIS / (ใช้/จ่าย/ส่งต่อ + หมดอายุ); นับถุงต้นทาง; ถุงที่ขาดวันหมดอายุยังไม่ยืนยันว่าใช้ก่อนหมดอายุ'],['KPI 3 ถุงที่ใช้แต่ไม่มีวันหมดอายุ',r.missingExpiryUsed],[],['เดือน','กาชาด Routine','Routine รับเข้าทั้งหมด','อัตรา']];
