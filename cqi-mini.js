...
async function registerMiniCqiPlan(event){
 ...
 const plan=editingId?(changed?await MinimumStockBackend.updateCqiPlan({...payload,evidenceRef:evidenceRef||'ภาพหลักฐาน',id:editingId,reason:String(f.get('edit_reason')||'').trim()}):previous):await MinimumStockBackend.registerCqiPlan({...payload,evidenceRef:evidenceRef||'ภาพหลักฐาน'});
 if(file){
   let uploadedPath='';
   try{uploadedPath=await MinimumStockBackend.uploadCqiEvidence(file);await MinimumStockBackend.attachCqiEvidence(plan.id,uploadedPath,String(f.get('edit_reason')||'').trim()||'แนบภาพประกอบแผน')}
...
function exportMiniCqiPlanTable(){
 ...
 const rows=[['แผนออกหน่วย',plan.outing_date,'สถานที่',plan.site],['เวลาทราบแผน',plan.decision_at,'หลักฐาน',plan.evidence_ref,'ชนิดผล',plan.evidence_kind==='reconstructed'?'คำนวณย้อนหลัง':'บันทึกขณะทราบแผน'],['สรุป',miniCqiPlanPassed(plan)?'ควรดำเนินตามแผน: พบหมู่เสี่ยง':forecast.canEvaluate?'ทบทวนความจำเป็น: ไม่พบหมู่เสี่ยง':'ข้อมูลไม่พอ'],[],['หมู่เลือด','Stock ต้นวัน','คาดใช้ก่อนออกหน่วย','คาดเหลือ','Minimum','ส่วนต่าง','ผล']];
...
function exportMiniCqiPlanChart(){
 ...
 const clone=svg.cloneNode(true);clone.setAttribute('xmlns','http://www.w3.org/2000/svg');
 const url=URL.createObjectURL(new Blob([new XMLSerializer().serializeToString(clone)],{type:'image/svg+xml;charset=utf-8'}));
 const img=new Image();img.onload=()=>{try{const canvas=document.createElement('canvas');canvas.width=1960;canvas.height=960;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,1960,960);ctx.drawImage(img,0,0,1960,960);const a=document.createElement('a');a.href=canvas.toDataURL('image/png');a.download=`CQI-KPI1-forecast-${plan.outing_date}-${plan.id.slice(0,8)}.png`;a.click()}finally{URL.revokeObjectURL(url)}};
 img.onerror=()=>{URL.revokeObjectURL(url);showModal('error','ส่งออกกราฟไม่สำเร็จ','ลองใหม่อีกครั้ง')};img.src=url;
}
